...
export const projectDetails: Record<string, ProjectDetail> = {
  ...
  "08-radiology-parser": {
    businessProblem: {
      headline: "Can a model read radiology impressions a keyword parser gets wrong?",
      paragraphs: ["Two synthetic Indonesian-language clients store 679 imaging impressions as free text, written 607 distinct ways across Bahasa Indonesia and Latin organ terms; about 60% are normal.", "A naive keyword parser is a fine first pass, but it over-flags negated normals like 'tidak membesar' (not enlarged). This project asks whether an NLP model can learn the context a keyword rule misses — and whether it truly generalizes across phrasings rather than memorizing them."],
      cards: [{ label: "Problem Type", value: "Clinical NLP — Multilingual" },
        { label: "Method", value: "Keyword baseline + TF-IDF model" },
        { label: "NLP Accuracy", value: "86% (held-out)" }],
    },
    dataSources: {
      headline: "Free-text impressions in word_value (two clients).",
      description: "Each imaging result carries a free-text impression (607 distinct phrasings across 679 records) plus a hidden rad_label ground truth, so the model is scored against an answer key it never sees as input.",
      tables: [{ name: "patient_service_results", fields: ["word_value", "rad_label", "service_name"], note: "Impression text + ground-truth label", primary: true },
        { name: "patient_services", fields: ["service_name", "status", "created_at"], note: "Service context" }],
    },
    methodology: {
      headline: "Naive baseline first, then a model — evaluated honestly.",
      description: "Establish a keyword baseline, then test whether a learned model beats it on held-out text.",
      steps: [{ step: "01", title: "Corpus Analysis", desc: "607 distinct impression strings catalogued across English and Bahasa, mixing normal and per-organ findings." },
        { step: "02", title: "Keyword Baseline", desc: "Flag abnormal when any finding term appears. Deliberately naive — no negation handling — as a fair floor." },
        { step: "03", title: "NLP Model", desc: "TF-IDF (uni- and bigrams) + logistic regression predicting normal vs abnormal from the impression text." },
        { step: "04", title: "Held-out Evaluation", desc: "75/25 stratified split. Only 9 identical strings appear in both sets, so test performance reflects real generalization." }],
    },
    analysis: {
      headline: "The model recovers the cases the keyword rule breaks on.",
      description: "Computed on the synthetic dataset.",
      kpis: [{ value: "86%", label: "NLP Accuracy", sub: "Held-out test set", color: "#27AE60" },
        { value: "0.80", label: "NLP F1", sub: "Abnormal class", color: "#1566FF" },
        { value: "68%", label: "Keyword Baseline", sub: "Naive, no negation", color: "#C0392B" },
        { value: "607", label: "Unique Phrasings", sub: "Of 679 impressions", color: "#C7AA50" }],
      charts: [{ title: "Keyword Baseline vs NLP Model", subtitle: "Accuracy on the held-out test set", unit: "%", data: [{ label: "Keyword baseline", value: 68 }, { label: "NLP model", value: 86 }] },
        { title: "Organ Mentions Across Impressions", subtitle: "Frequency of each organ group", unit: "", data: [{ label: "pulmo / lung", value: 300 }, { label: "ginjal / kidney", value: 289 }, { label: "cor / heart", value: 264 }, { label: "hepar / liver", value: 255 }, { label: "pleura", value: 230 }, { label: "vesica / gallbladder", value: 198 }] }],
    },
    insights: {
      headline: "Why the model beats the keyword rule.",
      items: [{ title: "Negation breaks naive keyword rules", desc: "Phrases like 'tidak membesar' and 'tidak tampak kardiomegali' contain finding words, so a keyword parser over-flags them as abnormal — capping the baseline near 68%." },
        { title: "The model learns context, not just words", desc: "TF-IDF + logistic regression reaches 86% accuracy (0.80 F1 on the abnormal class), recovering most negated-normal cases the keyword rule misses." },
        { title: "It generalizes, it doesn't memorize", desc: "Only 9 of 679 impression strings overlap between train and test, so held-out accuracy reflects genuine generalization across 607 distinct phrasings." },
        { title: "Reading note: synthetic impressions", desc: "Impression text is composed by a multilingual phrase generator, not drawn from real radiology reports." }],
    },
    future: {
      headline: "Where this goes next.",
      items: [{ title: "Negation-aware baseline", desc: "Add simple negation handling to the keyword parser for an interpretable fallback alongside the model." },
        { title: "Fine-tuned multilingual model", desc: "A model like IndoBERT would likely beat TF-IDF, especially on rarer findings." },
        { title: "Per-organ extraction", desc: "Move from document-level normal/abnormal to structured per-organ findings." },
        { title: "PACS integration", desc: "Link parsed findings to the actual images for a full workflow." }],
    },
  },
